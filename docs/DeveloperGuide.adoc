...
_Figure 3.1.1 Class Diagram of DeleteByNameCommand_

From _Figure 3.1.1_ the `DeleteByNameCommand` class is highly
...
**Alternative 1 (current choice):** Utilise `Streams` and create a helper class, `CaseInsensitiveExactNamePredicate`.

**Pros:**

* `Streams` improve code readability, allowing the function to be easily maintained
and updated by any developer.
...
* `Streams` have significantly much more overheads when compared to traditional methods
of iterating and filtering a list.
...
**Alternative 2:** Utilise traditional methods of iterating and filtering a list, coupled with `lambda` function.

**Pros:**

* Reduced overheads when iterating and filtering the list of persons in the Address Book.

* A `lambda` predicate removes the need for a separate class within the `Person` package.

**Cons:**

* Code is significantly harder to read and debug. This is attributed to a lack of abstraction.

* Code is not easily maintainable or updatable.
...
The filter command inherits from `Command` class as it is not an undoable command. The mechanism involves two main components: `Logic` and `Model`. The `Logic` component is in charge of parsing user inputs while `Model` updates the filtered person list based on the inputs.
...
`PersonHasTagPredicate` determines whether a person is eligible to appear in the filtered person list. It is implemented this way:
[source,java]
----
