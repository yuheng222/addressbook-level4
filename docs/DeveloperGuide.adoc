...
dependent on the `Person` package as well as the `Model` of the AB&B.
This dependency allows it to carry out its delete operation. The `Model` of the AB&B
will be directly updated within the command.

The self calls of `DeleteByNameCommand` are illustrated in the code fragment below:
```
@Override
