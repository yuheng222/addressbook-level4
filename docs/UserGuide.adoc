...
Address book's data can be stored in a backup file in the same directory as the original data file.
...
*A*: You may use the `backup` command to generate a backup file with the same format as the original data file.
